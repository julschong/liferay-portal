...
public class BaseDataSample implements DataSample, Serializable {
	...
	public void capture(RequestStatus requestStatus) {
		if ((_stopWatch != null) && (_duration == 0)) {
	...
	public String toString() {
		...
		if (_stopWatch != null) {
			...
			if (_duration != 0) {
				stopWatchString = simpleDateFormat.format(_duration);
			}
			else {
				stopWatchString = simpleDateFormat.format(_stopWatch.getTime());
			}
		}
		...
		return StringBundler.concat(
			...
			stopWatchString, ", timeout=", _timeout, ", user=", _user, "}");
	}
