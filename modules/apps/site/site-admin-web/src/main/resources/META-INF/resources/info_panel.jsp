...
%>'
							...
							<c:if test="<%= siteAdminDisplayContext.getOrganizationsCount(group) > 0 %>">
								<div>
									<aui:a href='<%= URLBuilder.create(assignMembersURL.toString()).addParameter("tabs1", "organizations").build() %>' label='<%= LanguageUtil.format(request, (siteAdminDisplayContext.getOrganizationsCount(group) == 1) ? "x-organization" : "x-organizations", siteAdminDisplayContext.getOrganizationsCount(group), false) %>' />
								</div>
							</c:if>

							<c:if test="<%= siteAdminDisplayContext.getUserGroupsCount(group) > 0 %>">
								<div>
									<aui:a href='<%= URLBuilder.create(assignMembersURL.toString()).addParameter("tabs1", "user-groups").build() %>' label='<%= LanguageUtil.format(request, (siteAdminDisplayContext.getUserGroupsCount(group) == 1) ? "x-user-groups" : "x-user-groups", siteAdminDisplayContext.getUserGroupsCount(group), false) %>' />
								</div>
							</c:if>
						</li>
