...
public class SearchResultsPortletSharedSearchContributor
	...
	private void _paginate(
		...
		Integer paginationDelta = null;

		if (paginationDeltaParameterValue != null) {
			paginationDelta = Integer.valueOf(paginationDeltaParameterValue);
		...
		int paginationDeltaValue = GetterUtil.getInteger(
			paginationDelta,
			searchResultsPortletPreferences.getPaginationDelta());

		portletSharedSearchSettings.setPaginationDelta(paginationDeltaValue);
		searchRequestBuilder.size(paginationDeltaValue);
	}
